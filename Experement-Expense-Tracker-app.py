...
with tab1:
    ...
    if not habits_df.empty:
        ...
        with h1:
            ...
            period_data = habits_df.groupby("TimePeriod")[amt_col].sum().reindex(period_order).fillna(0).reset_index()
        ...
        with h2:
            hourly = pd.DataFrame({"Hour": range(24)}).merge(habits_df.groupby("Hour")[amt_col].sum().reset_index(), how="left").fillna(0)
        ...
        with w1:
            weekday_data = habits_df.groupby("Weekday")[amt_col].sum().reindex(WEEK_ORDER).fillna(0).reset_index()
            fig = px.bar(weekday_data, x="Weekday", y=amt_col, template="plotly_dark", title="Total Spend by Day of Week")
            fig.update_layout(xaxis_fixedrange=True, yaxis_fixedrange=True)
            st.plotly_chart(fig, use_container_width=True, config=get_chart_config())
        
        with w2:
            weektype_data = habits_df.groupby("WeekType")[amt_col].sum().reset_index()
